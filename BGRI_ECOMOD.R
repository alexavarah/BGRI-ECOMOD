...
# This model is a farm/field level model developed to evaluate the economic consequences of changes in 
...
# Author: Dr Kwadjo Ahodo, for The Black Grass Resistance Initiative (BGRI)
# Created: 2017
...
solve_BGRI_ECOMOD <- function(filename,farmdata,default,farm="single",soil,rotlength,rotprob=NULL,crops,tillages,seedrate,delsowing,Nfert,Pfert,Kfert,bgherbdose,
  ...
  ECOMOD <- function(farm,default,soil,rotlength,rotprob,crops,tillages,seedrate,delsowing,Nfert,Pfert,Kfert,bgherbdose,
    ...
    if(length(machsize)>5){
      ...
      # The machine size must be set in the following order: Trator size (kW), roller size (m),
    ...
    if(soil%in%soi==TRUE){
      ...
      warning("Enter correct soil type: must be between 0.5 and 2.5 at an interval of 0.25. Check that values are formatted as 'numeric' in the .csv input file.")
    }
    
    cropBudget <- function(crop,asoil,subsidy,blackgrass,tillage,selfrot,rotpen,yieldoption){ #***********************************************************
      
      # This function estimates gross margin (£/ha) and grossprofit (£/ha) for each crop taking into
      ...
      # updated 09/08/2018 to include the average per hectare cost of herbicides NOT targeting BG, from the BGRI data
      ...
      # Winter wheat-blackgrass infestation assumption
      # Black-grass infestation level is ONLY taken into consideration if yield data are ESTIMATED,
      # based on the assumption that any effect of Black-grass would be manifest in 
      # the ACTUAL yield obtained by the farmer.
      ...
      if(crops[1]=="winterwheat"){
        ...
        if(yieldoption=="actual"){
          crd[c3,2] <- crd[c3,2] #-bgf(blackgrass[1])[2]-round(delsow(delsowing[1])[1]*crd[c3,2],2)
      ...
      if(crops[2]=="winterwheat"){
        ...
        crd[c6,3] <- seccost
        if(yieldoption=="actual"){
          crd[c3,3] <- crd[c3,3]#-bgf(blackgrass[2])[2]-round(delsow(delsowing[2])[1]*crd[c3,3],2)
      ...
      if(crops[3]=="winterwheat"){
        ...
        crd[c6,4] <- seccost
        if(yieldoption=="actual"){
          crd[c3,4] <- crd[c3,4]#-bgf(blackgrass[3])[2]-round(delsow(delsowing[3])[1]*crd[c3,3],2) 
      ...
      if(crops[4]=="winterwheat"){
        ...
        crd[c6,5] <- seccost
        if(yieldoption=="actual"){
          crd[c3,5] <- crd[c3,5]#-bgf(blackgrass[4])[2]-round(delsow(delsowing[4])[1]*crd[c3,5],2)
      ...
      if(crops[5]=="winterwheat"){
        ...
        crd[c6,6] <- seccost
        if(yieldoption=="actual"){
          crd[c3,6] <- crd[c3,6]#-bgf(blackgrass[5])[2]-round(delsow(delsowing[5])[1]*crd[c3,6],2)
      ...
      if(crops[6]=="winterwheat"){
        ...
        crd[c6,7] <- seccost
        if(yieldoption=="actual"){
          crd[c3,7] <- crd[c3,7]#-bgf(blackgrass[6])[2]-round(delsow(delsowing[6])[1]*crd[c3,7],2)
      ...
      Machines <- data.frame(
        ...
        #c(50000,7500,16000,21000,95000,14000,70000),
      ...
      #op <- kk#cropYield(nsoil, blackgrass)
      le <- 7
      if(subsidy=="no"){
        farmpayment <- 0
      }else{
      ...
      # Herbicide cost is assumed to be cost of herbicide cost (Cost of other chemical cost is assumed in Sundry cost)
      ...
      if("winterwheat"%in%crops==TRUE){
        ...
        wr[2,2] <- wr[2,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Plough
        wr[3,2] <- wr[3,3] <- round((0.06+0.00069*{cr$winterwheat[7]})+
                                      (58.82+41.5*{nsoil}+0.00626*{cr$winterwheat[7]})/tractor,2) # Plant
      ...
      if("springwheat"%in%crops==TRUE){
        ...
        wr[10,2] <- wr[10,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # plough
        wr[11,2] <- wr[11,3] <- round(((0.06+0.00069*{cr$springwheat[7]})+
                                         (58.82+41.5*{nsoil}+0.00626*{cr$springwheat[7]})/tractor),2)
      ...
      if("winterbarley"%in%crops==TRUE||"winteroats"%in%crops==TRUE){
        ...
        wr[18,2] <- wr[18,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Plough
      ...
      if("springbarley"%in%crops==TRUE||"springoats"%in%crops==TRUE){
        ...
        wr[26,2] <- wr[26,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Plough
      ...
      if("winterbeans"%in%crops==TRUE){
        ...
        wr[34,2] <- wr[34,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Plough
      ...
      if("springbeans"%in%crops==TRUE){
        ...
        wr[41,2] <- wr[41,3] <- round(((0.06+0.00069*{cr$springbeans[7]})+
                                         (92.42+0.00626*{cr$springbeans[7]}+41.5*{nsoil})/tractor),2) # Plant
        #wr[42,2] <- wr[42,3] <- wr[42,5] <- round(0.11+204.2/sprayer,2) # Spraying 
        if(crops[1]=="springbeans"){
      ...
      if("warepotatoes"%in%crops==TRUE){
        ...
        #wr[48,2] <- wr[48,3] <- wr[48,5] <- round(0.11+204.2/sprayer,2) # Spraying
        if(crops[1]=="warepotatoes"){
      ...
      if("wosr"%in%crops==TRUE){
        ...
        wr[55,2] <- wr[55,3] <- round(((0.387+0.00069*cr$wosr[7])+(99.42+0.00626*cr$wosr[7])/tractor),2)# Plant
        #wr[56,2] <- wr[56,3] <- wr[25,5] <- round(0.11+204.2/sprayer,2) # Spraying
        if(crops[1]=="wosr"){
      ...
      if("sugarbeet"%in%crops==TRUE){
        ...
        wr[61,2] <- wr[61,3] <- round((0.39+157/tractor), 2) # Plant
        #wr[62,2] <- wr[62,3] <- wr[62,5] <- round(0.11+204.2/sprayer,2) # Spraying
        if(crops[1]=="sugarbeet"){
        ...
        hvsbee <- round(((403/600)+2/(3*(1.25+0.51*{nsoil})*({cr$sugarbeet[c3]}/37.728))),2)
      ...
      if("sosr"%in%crops==TRUE){
        ...
        # round(((0.06+0.00069*cr$sosr[7])+(99.42+0.00626*cr$sosr[7])/tractor),2)# Plant
        wr[71,2] <- wr[71,3] <- round(((0.06+0.00069*cr$sosr[7])+(92.42+0.00626*(cr$sosr[7])+41.5*(nsoil))/tractor),2) # Plant
        #wr[56,2] <- wr[56,3] <- wr[25,5] <- round(0.11+204.2/sprayer,2) # Spraying
        if(crops[1]=="sosr"){
      ...
      if("winterlinseed"%in%crops==TRUE){
        ...
        # round(((0.06+0.00069*cr$winterlinseed[7])+(99.42+0.00626*cr$winterlinseed[7])/tractor),2)# Plant
        wr[77,2] <- wr[77,3] <- round(((0.06+0.00069*cr$winterlinseed[7])+(92.42+0.00626*(cr$winterlinseed[7])+41.5*(nsoil))/tractor),2) # Plant
        #wr[56,2] <- wr[56,3] <- wr[25,5] <- round(0.11+204.2/sprayer,2) # Spraying
        if(crops[1]=="winterlinseed"){
      ...
      if("springlinseed"%in%crops==TRUE){
        ...
        # round(((0.06+0.00069*cr$springlinseed[7])+(99.42+0.00626*cr$springlinseed[7])/tractor),2)# Plant
        wr[83,2] <- wr[83,3] <- round(((0.06+0.00069*cr$springlinseed[7])+(92.42+0.00626*(cr$springlinseed[7])+41.5*(nsoil))/tractor),2) # Plant
        #wr[56,2] <- wr[56,3] <- wr[25,5] <- round(0.11+204.2/sprayer,2) # Spraying
        if(crops[1]=="springlinseed"){
      ...
      if("driedpeas"%in%crops==TRUE||"peas"%in%crops==TRUE){
        ...
        #round((3*(0.114+0.00033*{cr$driedpeas[7]})+(54*{nsoil}+21.6)/tractor),2) # Sow
    ...
    #crops <- c("winterwheat","winterwheat","wosr","winterwheat","winterwheat","sugarbeet")
    #aa <- cropBudget(crops,soil,subsidy,blackgrass,tillage=tillages[1],selfrot=0,rotpen=0,yieldoption="estimate")
...
# 1. Define each of the model inputs. To do this, un-comment the code in lines 4100-4123 and change the values to your own *******
# 2. Change 'default' from "yes" to NULL in line 4125 *******
