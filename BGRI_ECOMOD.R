#********************************************************************************************************
#************************** BGRI ECONOMIC MODEL (BGRI-ECOMOD) *******************************************
#********************************************************************************************************

# This is a farm/field level model developed to evaluate the economic consequences of changes in 
# land use/farm management with a focus on Black-grass (Alopecurus myusoroides) management strategies.
# The model is capable of accommodating changes in farm management options such as crop types,
# length of rotation, tillage practices, seed rate and delayed sowing.
# It also incorporates yield penalties due to crop rotation/ sequence, delayed sowing 
# and level of Black-grass infestation.

# Authors: Dr Kwadjo Ahodo (main author) and Dr Alexa Varah (design), for The Black Grass Resistance Initiative (BGRI, http://bgri.info/)
# Created: 2017
# Funding: BBSRC and AHDB
# Contact: alexa.varah@ioz.ac.uk

rm(list=ls())

### MODEL CODE ====

solve_BGRI_ECOMOD <- function(filename,farmdata,default,farm="single",soil,rotlength,rotprob=NULL,crops,tillages,seedrate,delsowing,Nfert,Pfert,Kfert,bgherbdose,
  ...
  # Machine size (machsize) vector MUST be entered in the following order: 
  # 1. Tractor size (kW); 2. Roller size (m); 3. Power harrow size (m); 4. Sprayer tank size (litres); 5. Combine harvester size (kW)
  ...
  ECOMOD <- function(farm,default,soil,rotlength,rotprob,crops,tillages,seedrate,delsowing,Nfert,Pfert,Kfert,bgherbdose,
    ...
    if(is.null(default)){
      ...
      blackgrass <- c("low","low","low","low","low","low") 
      cropprice <- c(164,164,340,164,212,164)
      cropyield <- c(9.1,9,3.91,9.8,5.9,10)
      Nfertprice <- 0.80
      Pfertprice <- 0.63
      Kfertprice <- 0.47
      seedprice <- c(0.4,0.4,7.37,0.4,0.43,0.4)
      herbprice <- c(19.5,19.5,19.5,19.5,19.5,19.5) # An average from BGRI farm data
      glyphosateprice <- c(2.43,2.43,2.43,2.43,2.43,2.43) # An average from BGRI farm data
    ...
    ### SET ROTATION LENGTH AND CHECK INPUT DATA CONCURS ====
    
    # Rotation Length Assumption ====
    lcr <- 6 # rotlength = 6 years # Length of vector specifying years of rotation (max = 6 years)
    if(length(crops)>lcr){
    ...
    if(soil%in%soi==TRUE){
      ...
      warning("Enter correct soil type: must be between 0.50 and 2.50 at an interval of 0.25. Check that values retain 2 decimal places when read into the model: this may require formating them as 'text' in the .csv input file.")
    }
    

    ### CROP BUDGET FUNCTION ====
    ...
    cropBudget <- function(crop,asoil,subsidy,blackgrass,tillage,selfrot,rotpen,yieldoption){ 
      ...
      Input_Output <- c("N Fertiliser","P Fertiliser","K Fertiliser","N Fertiliser Price","P Fertiliser Price",
                        "K Fertiliser Price","Seed Rate","Seed Price","BG Herbicide Rate","BG Herbicide Price",
                        "Glyphosate Rate", "Glyphosate Price","Number of Sprays","Sugarbeet Transport","Fertiliser Cost","Seed Cost",
                        "Herbicide Cost","Sundry Cost","Sugarbeet Transport Cost","Primary Yield","Primary Yield Price","Secondary Yield",
                        "Secondary Yield Price","Subsidy","Farm Output","Variable Cost","Gross Margin","Fuel Cost","Labour Cost","Operating Cost",
                        "Gross Profit") # Crop input and output parameters
      ...
      # Assumed sundry costs informed by data in Nix (2014) and ABC (2014) (This can be updated depending on source of data)
      # The sundry cost includes cost of other chemicals such as fungicide, insecticides, growth regulators etc.
      # NB: the values given in lines 183 & 184 do NOT incorporate any herbicide costs.
      #snwwt <- 134; snswt <- 84; snwba <- 93; snsba <- 71; snwbe <- 89; snsbe <- 70; snwpo <- 1200; snwos <- 93; snsbt <- 321; snset <- 0;
      #snsos <- 58; sndpe <- 98; snwln <- 41; snsln <- 39; snwoa <- 67; snsoa <- 65; 
      
      # 09/08/2018: updated initial sundry costs (lines 183 & 184) to include the average per hectare cost of herbicides NOT specifically targeting BG, from BGRI data
      ...
      # Winter wheat yield penalty due to black-grass infestation ====
      # Black-grass infestation level is ONLY taken into consideration if yield data are ESTIMATED,
      # based on the assumption that any effect of black-grass would be manifest in the ACTUAL yield obtained by the farmer.
      ...
      # NB: IF ACTUAL YIELD DATA ARE USED, YIELD PENALTIES DUE TO DELAYED SOWING ARE NOT TAKEN INTO ACCOUNT
      ...
      seccost <- 60 # Secondary yield (straw) price assumed for cereal crops (wheat and barley crops)
      ...
      # Estimates work rate of farm operations, and fuel and labour costs
      
      # *Machine Sizes ====
      cr <- kk2
      ma <- modData[[3]][,seq(-1,-2)] # Machine data
      wr <- modData[[2]] # Work rate data 
      ...
      # The size of the combine harvester measured in tonnes/hour was derived on pro rata basis, based on information from
      ...
      combine <- round(combsize*extfactor) # Represents combine harvester size
      ...
      if("winterwheat"%in%crops==TRUE){
        ...
        wr[2,2] <- wr[2,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        wr[3,2] <- wr[3,3] <- round((0.06+0.00069*{cr$winterwheat[7]})+
                                      (58.82+41.5*{nsoil}+0.00626*{cr$winterwheat[7]})/tractor,2) # Sowing
        wr[4,2] <- wr[4,3] <- round((1.5/(tpspeed*rollwidth/10))*0.85,2) # Rolling
        wr[5,2] <- wr[5,3] <- 
      ...
      if("springwheat"%in%crops==TRUE){
        ...
        wr[9,2] <- wr[9,3] <- round(((0.06+0.00025*({cr$springwheat[3]}+{cr$springwheat[5]}))+
                                       (64.48+0.094*({cr$springwheat[3]}+{cr$springwheat[5]}))/ tsize),2)
        wr[10,2] <- wr[10,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        wr[11,2] <- wr[11,3] <- round(((0.06+0.00069*{cr$springwheat[7]})+
                                         (58.82+41.5*{nsoil}+0.00626*{cr$springwheat[7]})/tractor),2) # Sowing
        wr[12,2] <- wr[12,3] <- round((1.5/(tpspeed*rollwidth/10))*0.85,2) # Rolling
        ...
        cbswht <- round(2*(1.00*({cr$springwheat[c3]}+20)/4)/combine,2)
        wr[15,2] <- 2* cbswht; wr[15,3] <- 3* cbswht; wr[15,6] <- cbswht
        baswht <- round((({cr$springwheat[c5]}+13)/4)/combine*0.5,2) 
      ...
      if("winterbarley"%in%crops==TRUE||"winteroats"%in%crops==TRUE){
        ...
        wr[17,2] <- wr[17,3] <- round(((0.06+0.00025*({cr$winterbarley[3]}+{cr$winterbarley[5]}))+
                                         (64.48+0.094*({cr$winterbarley[3]}+{cr$winterbarley[5]}))/ tsize),2)
        wr[18,2] <- wr[18,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        wr[19,2] <- wr[19,3] <- round(((0.06+0.00069*{cr$winterbarley[7]})+
                                         (58.82+41.5*{nsoil}+0.00626*{cr$winterbarley[7]})/tractor),2) # Sowing
        wr[20,2] <- wr[20,3] <- round((1.5/(tpspeed*rollwidth/10))*0.85,2) # Rolling
        wr[21,2] <- wr[21,3] <- 
        ...
        bawbar <- round((({cr$winterbarley[c5]}+13)/4)/combine*0.5,2) 
      ...
      if("springbarley"%in%crops==TRUE||"springoats"%in%crops==TRUE){
        ...
        wr[25,2] <- wr[25,3] <- round(((0.06+0.00025*({cr$springbarley[2]}+{cr$springbarley[3]}))+
                                         (64.48+0.094*({cr$springbarley[2]}+{cr$springbarley[3]}))/ tsize),2)
        wr[26,2] <- wr[26,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        wr[27,2] <- wr[27,3] <- round(((0.06+0.00069*{cr$springbarley[7]})+
                                         (58.82+41.5*{nsoil}+0.00626*{cr$springbarley[7]})/tractor),2) # Sowing
        wr[28,2] <- wr[28,3] <- round((1.5/(tpspeed*rollwidth/10))*0.85,2) # Rolling
        ...
        cbsbar <- round(((1.15*{cr$springbarley[c3]}+24)/4)/combine,2) # Harvest
        wr[31,2] <- 2* cbsbar; wr[31,3] <- 3* cbsbar; wr[31,6] <- cbsbar
        basbar <- round((({cr$springbarley[c5]}+13)/4)/combine*0.5,2) # Baler
      ...
      if("winterbeans"%in%crops==TRUE){
        ...
        wr[33,2] <- wr[33,3] <- 
          round(((0.06+0.00025*({cr$winterbeans[3]}+{cr$winterbeans[5]}))+
                   (64.48+0.094*({cr$winterbeans[3]}+{cr$winterbeans[5]}))/ tsize),2) # P/K Fert
        wr[34,2] <- wr[34,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        wr[35,2] <- wr[35,3] <- round((3*(0.114+0.00033*{cr$winterbeans[7]})+
                                         (54*{nsoil}+21.6)/tractor),2) # Sowing
        wr[36,2] <- wr[36,3] <- round((1.5/(tpspeed*rollwidth/10))*0.85,2) # Rolling
        ...
        cbwbea <- round((4.05*({cr$winterbeans[c3]}+24)/4)/combine,2)
      ...
      if("springbeans"%in%crops==TRUE){
        ...
        wr[39,2] <- wr[39,3] <- 
          round(((0.06+0.00025*({cr$springbeans[2]}+{cr$springbeans[3]}))+
                   (64.48+0.094*({cr$springbeans[2]}+{cr$springbeans[3]}))/ tsize),2) # P/K fert
        wr[40,2] <- wr[40,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Plough
        wr[41,2] <- wr[41,3] <- round(((0.06+0.00069*{cr$springbeans[7]})+
                                         (92.42+0.00626*{cr$springbeans[7]}+41.5*{nsoil})/tractor),2) # Sowing
      ...
      if("warepotatoes"%in%crops==TRUE){
        ...
        wr[44,2] <- wr[44,3] <- 
          round(((1.80*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing 
        wr[45,2] <- wr[45,3] <- 
        ...
        wr[49,2] <- wr[49,3] <- round(((0.06+0.00025*({cr$warepotatoes[2]}+{cr$warepotatoes[3]}))+
                                         (64.48+0.094*({cr$warepotatoes[2]}+{cr$warepotatoes[3]}))/tsize),2) # Spread P/K 
        wr[50,2] <- wr[50,3] <- 
        ...
        hpot <- round(((403/600)+2/(3*(1.25+0.51*{nsoil})*({39.43}/37.728)))*2.51,2) # Harvest potatoes
      ...
      if("wosr"%in%crops==TRUE){
        ...
        wr[53,2] <- wr[53,3] <- round(((0.06+0.00025*({cr$wosr[2]}+{cr$wosr[3]}))+
                                         (64.48+0.094*({cr$wosr[2]}+{cr$wosr[3]}))/ tsize),2)
        wr[54,2] <- wr[54,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        wr[55,2] <- wr[55,3] <- round(((0.387+0.00069*cr$wosr[7])+(99.42+0.00626*cr$wosr[7])/tractor),2)# Sowing
      ...
      if("sugarbeet"%in%crops==TRUE){
        ...
        wr[61,2] <- wr[61,3] <- round((0.39+157/tractor), 2) # Sowing
        ...
        wr[63,2] <- wr[63,3] <- round(((0.06+0.00025*({cr$sugarbeet[2]}+{cr$sugarbeet[3]}))+
                                         (64.48+0.094*({cr$sugarbeet[2]}+{cr$sugarbeet[3]}))/ tsize),2) # Spread P/K 
        wr[64,2] <- wr[64,3] <- 
      ...
      if("setaside"%in%crops==TRUE){
        # work rates for setaside operations ====
        wr[67,2] <- wr[67,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
      ...
      if("sosr"%in%crops==TRUE){
        ...
        wr[69,2] <- wr[69,3] <- round(((0.06+0.00025*({cr$sosr[2]}+{cr$sosr[3]}))+
                                         (64.48+0.094*({cr$sosr[2]}+{cr$sosr[3]}))/ tsize),2)
        wr[70,2] <- wr[70,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
        
        wr[71,2] <- wr[71,3] <- round(((0.06+0.00069*cr$sosr[7])+(92.42+0.00626*(cr$sosr[7])+41.5*(nsoil))/tractor),2) # Sowing
        ...
        cbsosr <- round(((4.05*({cr$sosr[c3]}+24)/4)/combine),2)
      ...
      if("winterlinseed"%in%crops==TRUE){
        # work rates for winter linseed operations ====
        wr[75,2] <- wr[75,3] <- round(((0.06+0.00025*({cr$winterlinseed[2]}+{cr$winterlinseed[3]}))+
                                         (64.48+0.094*({cr$winterlinseed[2]}+{cr$winterlinseed[3]}))/ tsize),2)
        wr[76,2] <- wr[76,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
       
        wr[77,2] <- wr[77,3] <- round(((0.06+0.00069*cr$winterlinseed[7])+(92.42+0.00626*(cr$winterlinseed[7])+41.5*(nsoil))/tractor),2) # Sowing
        ...
        cbwlin <- round(((4.05*({cr$winterlinseed[c3]}+24)/4)/combine),2)
      ...
      if("springlinseed"%in%crops==TRUE){
        # work rates for spring linseed operations ====
        wr[81,2] <- wr[81,3] <- round(((0.06+0.00025*({cr$springlinseed[2]}+{cr$springlinseed[3]}))+
                                         (64.48+0.094*({cr$springlinseed[2]}+{cr$springlinseed[3]}))/ tsize),2)
        wr[82,2] <- wr[82,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Ploughing
       
        wr[83,2] <- wr[83,3] <- round(((0.06+0.00069*cr$springlinseed[7])+(92.42+0.00626*(cr$springlinseed[7])+41.5*(nsoil))/tractor),2) # Sowing
        ...
        cbslin <- round(((4.05*({cr$springlinseed[c3]}+24)/4)/combine),2)
      ...
      if("driedpeas"%in%crops==TRUE||"peas"%in%crops==TRUE){
        ...
        wr[88,2] <- wr[88,3] <- round(((1.44*(50*{nsoil}+20))/tractor)*wkb,2) # Plough
        
        wr[89,2] <- wr[89,3] <- round((0.06+0.00069*{cr$driedpeas[7]})+(92.42+0.00626*{cr$driedpeas[7]}+41.5*{nsoil})/tractor,2) # Sowing
        ...
        cbdpea <- round((4.05*({cr$driedpeas[c3]}+24)/4)/combine,2)
      ...
      ## Estimate fuel and labour costs ====
    ...
    if(is.null(rotprob)){
      rotp <- 1/nrotl # Rotation probability
    }else{
      rotp <- rotprob
    }
    ...
    if(crop2=="winterwheat"){ 
      ...
      if(crop1=="warepoatoes"){
        selfrot <- 100 # Not allowed / strongly discouraged
  ...
  if(farm=="single"){
    ...
    # Create column headers with .1, .2 etc after the description - 
    # this is to allow subsequent use of re-shape (dplyr library) to transform output data files from wide to long if required
    cp1 <- "crop.1"
    cp2 <- "crop.2"
    cp3 <- "crop.3"
    cp4 <- "crop.4"
    ...
    for(i in index){
      fms <- c(fms,ECOMOD(farm="multiple",default,soil=fd[i,3],rotlength,rotprob,crops=c(cp[,1][i],cp[,2][i],cp[,3][i],cp[,4][i],cp[,5][i],cp[,6][i]),
                          ...
                          cropyield=as.numeric(fd[i,seq(78,83)]),yieldoption=yo[i],  
      ...
      # NAME OF THE OUTPUT FILE CAN BE CHANGED TO WHICHEVER NAME DESIRED *********
      # e.g. "OUTPUT.csv" as in Supplementary Information, Section 3.6.
...
### TO RUN MODEL FOR A SINGLE FARM OR FIELD ====

# To ensure the model generates some results, default model inputs have been incorprated into the model codes. 
# To run the model with one's own model inputs, the user needs to do three things:
# 1. Define each of the model inputs. To do this, un-comment the code in lines 4103-4126 and change the values to your own *******
# 2. Change 'default' from "yes" to NULL in line 4128 *******
# 3. ALWAYS SET filename AND farmdata TO NULL IF MODEL IS TO BE RUN FOR A SINGLE FIELD OR FARM *******
...
# glyphosatedose <- c(2,1.6,4,2.23,4.1,0.98) 
# numberofsprays <- c(3,3,4,4,6,4)
# blackgrass <- c("low","medium","low","low","low","medium") 
# cropprice <- c(164,164,340,164,212,164)
# cropyield <- c(9.1,9,3.91,9.8,5.9,10)
# Nfertprice <- 0.80
# Pfertprice <- 0.63
...
sfm <- solve_BGRI_ECOMOD(filename=NULL,farmdata=NULL,default="yes",farm="single",soil=2.5,rotlength=6,rotprob=NULL,crops,tillages,seedrate,delsowing,Nfert,Pfert,Kfert,bgherbdose,
                         glyphosatedose,numberofsprays,subsidy="yes",blackgrass,croprice,cropyield,yieldoption="actual",Nfertprice,
                         Pfertprice,Kfertprice,seedprice,herbprice,glyphosateprice,machsize,fuelprice,labourwage)


### TO RUN MODEL FOR MULTIPLE FARMS OR FIELDS ====

# 1. Set the directory which contains the data file
workingdir <-"Your/file/path"
setwd(workingdir)

# 2. Decide the file name e.g. "OUTPUT.csv"
fn <- "OUTPUT.csv"

# 3. Call the data file
fd <- suppressWarnings(read.csv("INPUT.csv"))

# 4. Load data file into model by equating it to farmdata.
# 5. Set the rotation length.
# 6. Set NEW rotation probability (rotprob) if desired, otherwise set to NULL. By setting rotprob to NULL default rotation probability is estimated based on rotation length.
# 7. Choose whether you are running the model using measured (actual) yields, or whether you want the model to estimate yield for you (input is "actual" or "estimate").
# 8. ALWAYS SET default TO NULL WHEN RUNNING MODEL FOR MULTIPLE FIELDS. 

mfm <- solve_BGRI_ECOMOD(filename=fn,farmdata=fd,default=NULL,farm="multiple",soil=2.5,rotlength=6,rotprob=1/6,crops,tillages,seedrate,delsowing,Nfert,Pfert,Kfert,bgherbdose,
                         glyphosatedose,numberofsprays,subsidy="yes",blackgrass,cropprice,cropyield,yieldoption="estimate",Nfertprice,
                         Pfertprice,Kfertprice,seedprice,herbprice,glyphosateprice,machsize,fuelprice,labourwage)
